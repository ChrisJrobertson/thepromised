...
export function LetterWizard({
  ...
  const isFirstLetter = lettersUsed === 0;
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (!response.ok) {
        // Credits exhausted server-side — show soft paywall
      ...
      // Update credit counter from response
      if (data.creditsUsed !== undefined && data.creditsLimit !== undefined) {
        setCreditsInfo({ used: data.creditsUsed, limit: data.creditsLimit });
      }
      ...
      // Post-magic-moment nudge: shown after a free user's first successful AI letter
      if (tier === "free" && isFirstLetter) {
        toast.success(
          "Your letter is ready. Free users get 1 AI letter per month — upgrade for more.",
          { duration: 7000 }
        );
      }
    } catch {
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        {creditsInfo.limit > 0 && step !== "generating" && (
          <div className="flex items-center justify-between rounded-md border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5">
              ...
              {creditsInfo.used} of {creditsInfo.limit} AI letter{creditsInfo.limit === 1 ? "" : "s"} used this month
            </span>
            {tier === "free" && creditsInfo.used >= creditsInfo.limit && (
              <a className="font-medium text-primary underline underline-offset-2" href="/pricing">
                Upgrade →
              </a>
            )}
          </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                >
                  ...
                  {isExhausted ? "Monthly limit reached — upgrade" : "Generate Letter with AI"}
                </Button>
              </div>
            </CardContent>
          </Card>
